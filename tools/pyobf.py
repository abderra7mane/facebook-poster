    ...
    def simple(self):
        ...
        for i in xrange(len(s)):
            ...
            if indent_size > 0:
                line = s[i]
                instr = line.strip()
                indent = line[:line.index(instr)]
                indent = indent.replace(" " * indent_size, "\t")
...
if __name__ == '__main__':
	...
	if len(sys.argv) != 3:
		print "usage : %s <input-file> <output-file>" % sys.argv[0].split(os.path.sep)[-1]
		sys.exit(1)
	
	open(sys.argv[2], 'w').write(Obfuscator(open(sys.argv[1]).read()).build_simple())
